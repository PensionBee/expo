...
async function copyInitialShellAppFilesAsync(androidSrcPath, shellPath, isDetached, sdkVersion) {
  ...
  await copyToShellApp('versioning_linking.gradle');
...
exports.createAndroidShellAppAsync = async function createAndroidShellAppAsync(args) {
  ...
  /* testEnvironment */
  'none', buildFlags, url, releaseChannel, null);
...
async function runShellAppModificationsAsync(context, sdkVersion, buildMode) {
  ...
  const updatesCheckAutomaticallyDisabled = manifest.updates && manifest.updates.checkAutomatically === 'ON_ERROR_RECOVERY';
  ...
  if (isDetached) {
    ...
    await Promise.all(['MainActivity.java', 'MainActivity.kt'].map(file => {
      const target = _path().default.join(shellPath, 'app', 'src', 'main', 'java', 'host', 'exp', 'exponent', file);

      return _fsExtra().default.existsSync(target) ? regexFileAsync('TEMPLATE_INITIAL_URL', url, target) : Promise.resolve();
    }));
  ...
  await regexFileAsync('"app_name">Expo<', `"app_name">${xmlWeirdAndroidEscape(name)}<`, _path().default.join(shellPath, 'app', 'src', 'main', 'res', 'values', 'strings.xml'));
  await regexFileAsync('"app_name">Expo Go<', `"app_name">${xmlWeirdAndroidEscape(name)}<`, _path().default.join(shellPath, 'app', 'src', 'main', 'res', 'values', 'strings.xml'));
  await regexFileAsync('"versioned_app_name">Expo Go<', `"versioned_app_name">${xmlWeirdAndroidEscape(name)}<`, _path().default.join(shellPath, 'app', 'src', 'main', 'res', 'values', 'strings.xml'));
  await regexFileAsync(/^\s*<string name="unversioned_app_name">Expo Go \(unversioned\)<\/string>\s*$/m, '', _path().default.join(shellPath, 'app', 'src', 'main', 'res', 'values', 'strings.xml')); // Splash Screen background color
...
async function removeObsoleteSdks(shellPath, requiredSdkVersion) {
  let multipleVersionReactNativeActivity = _path().default.join(shellPath, 'expoview/src/versioned/java/host/exp/exponent/experience/MultipleVersionReactNativeActivity.java'); // fallback to non-versioned for backward compatibility


  if (!_fsExtra().default.existsSync(multipleVersionReactNativeActivity)) {
