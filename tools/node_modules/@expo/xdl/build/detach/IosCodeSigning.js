...
function _ensureBundleIdentifierIsValid(plistData, expectedBundleIdentifier) {
  ...
  if (!(0, _minimatch().default)(expectedBundleIdentifier, actualBundleIdentifier)) {
...
async function createEntitlementsFile({
  ...
  const entitlementsPaths = (0, _glob().sync)('Products/Applications/*.app/*.entitlements', {
    absolute: true,
    cwd: archivePath
  });
  ...
  const archiveEntitlementsRaw = await _fsExtra().default.readFile(archiveEntitlementsPath, 'utf8');
  let archiveEntitlementsData;

  try {
    archiveEntitlementsData = _plist().default.parse(archiveEntitlementsRaw);
  } catch (error) {
  ...
  let generatedEntitlements = (0, _omit().default)(entitlements, blacklistedEntitlementKeys);

  if (!manifest.ios.usesIcloudStorage) {
    generatedEntitlements = (0, _omit().default)(generatedEntitlements, blacklistedEntitlementKeysWithoutICloud);
  } else {
  ...
  if (!manifest.ios.associatedDomains) {
    generatedEntitlements = (0, _omit().default)(generatedEntitlements, 'com.apple.developer.associated-domains');
  }

  if (!manifest.ios.usesAppleSignIn) {
    generatedEntitlements = (0, _omit().default)(generatedEntitlements, 'com.apple.developer.applesignin');
  }
  ...
  const generatedEntitlementsPlistData = _plist().default.build(generatedEntitlements);
