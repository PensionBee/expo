...
Object.defineProperty(exports, "setBundleIdentifier", {
  enumerable: true,
  get: function () {
    return _IosSetBundleIdentifier().setBundleIdentifier;
  }
});
...
function _IosSetBundleIdentifier() {
  const data = require("./IosSetBundleIdentifier");

  _IosSetBundleIdentifier = function () {
    return data;
  };
  ...
  if (context.type === 'service') {
    ...
    if (process.env.EXPO_VIEW_DIR) {
      ...
      if (!(0, _ExponentTools().isDirectory)(expoRootTemplateDirectory)) {
        ...
        (0, _assert().default)(iosExpoViewUrl, `The URL for ExpoKit iOS must be set`);
...
async function _renameAndMoveProjectFilesAsync(context, projectDirectory, projectName) {
  ...
  filesToMove.forEach(async fileName => {
    const destFileName = _path().default.join(_path().default.dirname(fileName), `${projectName}${_path().default.extname(fileName)}`);
...
async function _renderPodfileFromTemplateAsync(context, expoRootTemplateDirectory, sdkVersion, iosClientVersion) {
  ...
  if (context.type === 'user') {
    (0, _assert().default)(iosClientVersion, `The iOS client version must be specified`);
...
async function createDetachedAsync(context) {
  ...
  _Logger().default.info('Installing packages...');

  await (0, _installPackagesAsync().default)(projectRootDirectory);
...
async function getNewestSdkVersionSupportedAsync(context) {
  if (context.type === 'user') {
    ...
    allVersions.forEach(version => {
      const majorComponent = (0, _ExponentTools().parseSdkMajorVersion)(version);
